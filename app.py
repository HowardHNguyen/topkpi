...
# Streamlit app (single file, no subfolders) — ready for Streamlit Cloud
...
# ------------------------------
st.set_page_config(
    page_title="MaxAIS • KPI + Propensity",
    page_icon="📈",
    layout="wide",
)
...
# If your training pipeline used a specific cleaned set of features (Cell 4C final),
# the saved pipeline contains a ColumnTransformer listing those column names.
def _expected_columns_from_pipeline(pipe) -> list:
    try:
        ...
        return list(dict.fromkeys(cols))  # unique order-preserving
    except Exception:
        return None
...
def ensure_columns_for_pipeline(df: pd.DataFrame, expected: list) -> pd.DataFrame:
    """Make sure df has the exact columns the pipeline expects (add missing as NaN; keep dtypes best-effort)."""
    if expected is None:
        return df  # fallback; the pipeline may handle arbitrary columns
    out = df.copy()
    for c in expected:
        if c not in out.columns:
            out[c] = np.nan
    # Keep only expected columns (model was trained on this exact set)
    out = out[expected]
    return out

def proxy_clv(df: pd.DataFrame) -> pd.Series:
    """
    If CLV column is absent, build a simple proxy:
    proxy = 12 * ARPU * margin; where ARPU approximated by 'Monthly Premium Auto'
    Adjust constants if you prefer.
    """
    monthly = df.get("Monthly Premium Auto", pd.Series(np.nan, index=df.index)).astype(float)
    # conservative margin & months
    months = 12.0
    margin = 0.35
    prox = months * monthly * margin
    # safety: if missing column, fallback to global median once computed
    if prox.isna().all():
...
def compute_kpis(df: pd.DataFrame,
    ...
    # Estimated acquisition cost per converted record
    ...
    # CLV series (real or proxy)
    if clv_col in d.columns:
        clv = d[clv_col].clip(lower=0)
    else:
        clv = proxy_clv(d)
    ...
    out = {
        "overall": {
            ...
            "acquired": acquired,
            "cpa": cpa,
            "clv_realized": float(clv_realized),
            "roi": roi
        }
    }
...
with st.sidebar.expander("Cost per Acquisition (override)", expanded=False):
    web = st.number_input("Web", value=DEFAULT_COST_MAP["Web"], min_value=0.0, step=5.0)
    cc  = st.number_input("Call Center", value=DEFAULT_COST_MAP["Call Center"], min_value=0.0, step=5.0)
    br  = st.number_input("Branch", value=DEFAULT_COST_MAP["Branch"], min_value=0.0, step=5.0)
    ag  = st.number_input("Agent", value=DEFAULT_COST_MAP["Agent"], min_value=0.0, step=5.0)
...
if os.path.exists(MODEL_PATH):
    ...
    st.sidebar.warning("best_model.pkl not found in repo root. Upload it to enable scoring.")
...
# ------------------------------
# Guard: target presence (for KPI truth)
# ------------------------------
...
if section == "📊 KPIs":
    ...
    if df is None:
        ...
        # If target missing, we can still show CPA estimates & ROI using predicted conversions (below, after scoring).
        if has_target:
        ...
        if has_target:
            ...
            if isinstance(by_ch, pd.DataFrame) and len(by_ch):
                ...
                fig = px.bar(by_ch, x="Sales Channel", y="conversion_rate", title="Conversion Rate by Channel", text=by_ch["conversion_rate"].map(lambda v: f"{100*v:.1f}%"))
            ...
            if isinstance(by_offer, pd.DataFrame) and len(by_offer):
                ...
                fig2 = px.bar(by_offer, x="Renew Offer Type", y="roi", title="ROI by Offer Type", text=by_offer["roi"].map(lambda v: f"{100*v:.1f}%"))
                st.plotly_chart(fig2, use_container_width=True)

# ------------------------------
# Section: Propensity (scoring)
# ------------------------------
    ...
    if df is None:
        ...
        st.warning("Model not loaded. Put best_model.pkl in repo root.")
    else:
        ...
        # If no ground-truth conversion column, we can show estimated KPIs using a threshold
        ...
        # Estimated KPI block (if no actual conversion)
...
# ------------------------------
    ...
    if df is None:
        ...
        # Quantile bins for observed vs predicted
...
# ------------------------------
st.markdown("---")
st.caption("© Howard Nguyen, PhD • KPI dashboard and calibrated propensity model. Upload CSVs with the same schema used for training. If CLV is missing, a proxy is used for ROI.")
